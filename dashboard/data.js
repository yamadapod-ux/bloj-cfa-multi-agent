...
`
...

  {
    ...
    forwardSharpe: null,
    maxDrawdown: -0.69,
    kelly: "0% — ไม่มี position size ที่ mathematically-optimal (P-W EV ต่ำกว่าราคา 65-74% ทุก analyst; permanent capital impairment prob ~55-65% ถ้าเข้าที่ $92)",
    moat: "Narrow (eroding) — network moat บน USDC transaction-standard (~70% volume share) มีจริง ~5-10 ปี แต่ ~95% ของ revenue อยู่บน reserve-income layer ที่ไม่มี moat เลย (exogenous to Fed rate + ถูก compete away). Distribution moat: ไม่แข็ง (Circle เช่า distribution จาก Coinbase ที่หนุน Open USD คู่แข่ง)",
    esgRating: "Medium-High",
    esgScore: 5.6,
    macroRegime: "RISK-ON",
    morganQA: "CONDITIONAL PASS (0 HIGH, 3 MEDIUM, 3 LOW, Data Quality 7.6/10) — Leo แก้ Issue #2 (ESG structured data -> reconciled 5.6, e7/s5.5/g4.5) ก่อน embed; Issues #1/#3/#4/#5/#6 Charlie แก้ใน report แล้ว. Reject type: FORMAT-dominant + partial SANITY_FAIL (WACC guardrail) + minor SOURCE_MISSING (Street PT)",
    sp500Reference: 7730.99,
    bucket: "Growth",
    revenueGrowthYoY: 37.19,
    entryZone: "$24-34 (re-entry watch) — เฉพาะเมื่อ Bull-Flip trigger ยิง (circulation inflection + Arc traction); ideally <= $28",
    positionSize: "0% — ไม่ผ่าน Conviction Gate (>=6.5 Growth) และ Growth MOS Gate (FAIL 0/2: Reverse DCF implied 35% >> 1.2x consensus; Fwd P/E ~70x = +52% premium peer median)",
    keyThesis: "CRCL ออก USDC (stablecoin #2 ของโลก) แต่ ~95% ของรายได้คือดอกเบี้ยบน ~$73B reserve float ซึ่งเป็น function ของ Fed rate ที่กำลังลด ในช่วงที่ USDC circulation หด QoQ — priced for perfection at the peak of the rate cycle (MU analog 2026-08-27). ราคา $92.26 สูงกว่า Blended FV $26 ถึง 3.5 เท่า และสูงกว่า bull case ($48) เกือบ 2 เท่า, Growth MOS Gate FAIL 0/2, Conviction 5.67 < 6.5 gate",
    topRisk: "Peak rate-cycle earnings — Reserve Return Rate -66bps YoY แล้ว + Fed cuts priced 12M fwd ทำให้ reserve income YoY อาจติดลบ ขณะที่ตลาด capitalize peak economics ที่ Fwd P/E ~70x -> multiple compression ไป 20-30x = -50% ถึง -65% โดยไม่ต้องมี earnings miss. ซ้อนด้วย distribution disintermediation จาก Open USD consortium (Coinbase co-founded, 140+ partners)",
    reportFile: "../reports/CRCL_2026-08-28.md",
    businessSummary: {
    ...
    esgBreakdown: { e: 7, s: 5.5, g: 4.5, overall: 5.6 },
    customerConcentration: {
    ...
    revenueSegments: [
      { label: "Reserve income (float ดอกเบี้ยบน USDC reserves)", value: 95 },
      { label: "Other (API fees / RWA / ARC token presale)", value: 5 }
    ],
    competitorData: [
      { ticker: "CRCL", company: "Circle Internet Group (subject)", revenueGrowth: 37.19, operatingMargin: 7.42, marketCap: 23400 },
      { ticker: "COIN", company: "Coinbase Global", revenueGrowth: -9.24, operatingMargin: 7.38, marketCap: 49250, strengths: "คุม distribution + end-user; ได้ส่วนแบ่ง reserve income จาก Circle โดยตรง (asymmetric); รายได้หลากหลาย", weaknesses: "Revenue -9.2% TTM; ขาดทุน TTM (ROE -7.9%); earnings ผันผวนตาม crypto cycle", threatLevel: "HIGH" },
      { ticker: "PYPL", company: "PayPal Holdings", revenueGrowth: 5.69, operatingMargin: 17.61, marketCap: 46350, strengths: "Distribution network 400M+ users / 70 ตลาด; PYUSD ฝังใน checkout; profitable จริง (ROE 24.5%); valuation ถูก (Fwd P/E 11x)", weaknesses: "PYUSD supply ยังเล็กมาก; onchain/DeFi traction ต่ำ; ยังไม่ prioritize stablecoin เป็น core", threatLevel: "MEDIUM" },
      { ticker: "HOOD", company: "Robinhood Markets", revenueGrowth: 38.27, operatingMargin: 46.03, marketCap: 98430, strengths: "Rev +38% TTM, op margin 46%, ROE 23.6%; crypto + brokerage flywheel", weaknesses: "ไม่ออก stablecoin เอง (สมาชิก consortium อื่น); business = trading-driven", threatLevel: "LOW" },
      { ticker: "USDT", company: "Tether (private)", revenueGrowth: null, operatingMargin: null, marketCap: null, strengths: "Supply share ~59% (~$185B, 2.5x USDC); กำไรต่อหน่วยสูงมาก (ไม่มี Coinbase-style split); brand = the dollar ในตลาดเกิดใหม่", weaknesses: "Reserve transparency ต่ำกว่า (ไม่มี full audit); US regulatory risk สูง", threatLevel: "HIGH" },
      { ticker: "OPENUSD", company: "Open USD consortium (140+ บริษัท)", revenueGrowth: null, operatingMargin: null, marketCap: null, strengths: "Distribution reach รวมกันเหนือทุกคน (Visa/MC/Stripe/Coinbase/Shopify/BlackRock); แบ่ง reserve income ให้พันธมิตรมากกว่า Circle", weaknesses: "ยังใหม่มาก, supply ยังไม่มีนัย; governance-by-committee ช้า; ยังไม่พิสูจน์ economics", threatLevel: "HIGH" }
    ],
    bullFlipTriggers: [
      "ราคา <= $34 โดย thesis ไม่แย่ลง (MOS กลับมา > 15-20% vs ESG-adj FV; ideally <= $28)",
      "USDC circulation กลับมาโต QoQ 2 ไตรมาสติด + supply share ทะลุ 26-27%",
      "Arc Mainnet post-launch: non-reserve revenue run-rate > $400M annualized ภายใน 12 เดือน",
      "Fed หยุด cut cycle — dot plot terminal >= 3.25% -> reserve yield floor สูงขึ้น",
      "Circle ได้ distribution deal ใหม่ ที่เก็บ > 60-65% ของ reserve income (ดีกว่า Coinbase)"
    ],
    bearFlipTriggers: [
      "USDC circulation < $65B (จาก $73.3B) ก่อนสิ้นปี 2026",
      "Reserve income YoY ติดลบ ในไตรมาสใดไตรมาสหนึ่ง",
      "Open USD consortium supply > $15-20B ภายใน 12 เดือน",
      "CLARITY Act ผ่าน โดยจำกัด yield economics ของ issuer",
      "Coinbase ประกาศ default stablecoin เป็นตัวอื่น / ขึ้น revenue-share demand",
      "Fwd P/E > 55x ขณะ consensus revenue ปรับลง > 15%"
    ],
    thesisInvalidation: "Non-reserve revenue > 30% ของ total -> business model เปลี่ยนจาก rate play เป็น infrastructure platform -> re-value ทั้งหมด | Fed pivot กลับไปขึ้นดอกเบี้ย -> reserve income model กลับมา tailwind | M&A — Circle ถูกซื้อ / ซื้อ distribution partner",

    fullContent: `# 🪙 CRCL — Circle Internet Group, Inc.
### Equity Research Report · บลจ. CFA Multi-Strategy Aggressive Growth
