...
| FSLR | First Solar, Inc. | Energy / Solar Manufacturing (US-based, IRA domestic-content moat) | 2026-09-03 | $200.78 (2026-09-01/02, 2 sources: aggregator via Yahoo/Fool $199.65 + CNBC/aggregator $201.90, 0.6% diff — flag re-verify ≥2 clean sources ก่อน full analysis) | ~6.3% (EV/FCF 15.85x implied) | Q1 2026 net sales +24% YoY, net income +65% YoY (ยังไม่ verify TTM rev growth% session นี้) | ROIC 12.4-21.7% (source variance — reconcile ตอน full analysis) / ROE ยังไม่ verify | **VALUE-FIRST clean pass (A+B+C) — Round 27 exception scan (Scout Pause ยังมีผลถึง Sept FOMC, CIO อนุมัติ exception 2026-09-03).** ราคาลง **37.7% จาก 52W high $320.95** (52W low $182.99) แม้ fundamentals แข็ง: net cash $1.7B, debt/FCF 0.28x (แข็งแกร่งมาก), record backlog, GAAP-profitable, Q1 net income +65% YoY. ตลาดกดทั้ง solar sector จาก policy overhang (residential tax-credit expiration + Chinese oversupply fear) แต่ FSLR เป็น utility-scale US-manufacturer ที่ได้ประโยชน์จาก IRA domestic-content requirement (คู่แข่งจีนโดน tariff กีดกัน) — sentiment ที่กดทั้ง sector อาจไม่ตรงกับ FSLR โดยเฉพาะ = potential sentiment-fundamental disconnect. Sector ใหม่ทั้งหมด (Solar Manufacturing — ไม่ overlap ตำแหน่งปัจจุบัน). ⚠️ ROIC มี source variance มาก (12.4%/13.7%/21.7%) — บาง source ระบุ "ROIC ไม่คุ้ม WACC" ต้อง reconcile เลขจริงตอน full analysis ก่อนตั้ง conviction. Entry Zone (prelim) $180-210 (mid $195) · Stop (prelim) $156 (-20%) | **NEW — TOP PICK Round 27 — รอ Analysis (VALUE-FIRST exception screen) — Target date 2026-09-10** |
| CNC | Centene Corporation | Healthcare / Managed Care (Medicaid + ACA Marketplace) | 2026-09-03 | $43.68 (2026-09-02, stockanalysis.com — single-source, flag re-verify ≥2 clean sources ก่อน full analysis) | EV/FCF 3.13x implied (~32% equivalent yield — ผิดปกติสูง ต้อง verify FCF quality/normalize ก่อนเชื่อ) | ยังไม่ verify revenue growth% session นี้ | ROIC 4.32% (ต่ำกว่า WACC ทั่วไป ~9-10%) / P/B 1.20x | **CONDITIONAL VALUE-FIRST (A+B ผ่านชัด, C อ่อน) — Round 27 exception scan.** ราคาลง **33.8% จาก 52W high $66.03** (52W low $25.08), P/B 1.20x ถูกกว่า sub-sector median ตาม OSCR precedent (insurer ปกติ 1-2x ตาม through-cycle ROE). แต่ ROIC 4.32% ต่ำกว่า WACC ชัดเจน + บริษัทมีประวัติถอน/ปรับ guidance จากปัญหา ACA marketplace risk-adjustment mispricing ปี 2025 — ต้อง Bear ตรวจ quality-of-earnings หนักแบบเดียวกับ OSCR/CHYM ไม่ใช่ clean pass เหมือน FSLR. EV/FCF 3.13x ต่ำผิดปกติเทียบ peer MOH (35.33x ที่ scan รอบนี้พบเช่นกันแต่ไม่ผ่านเกณฑ์ดีเท่า CNC จึงไม่เพิ่มเข้า table) — อาจสะท้อน one-time item หรือ premium-float distortion ต้อง normalize ก่อนเชื่อเลข. Sector ซ้ำ Managed Care (มี ELV ใน pending + OSCR archived AVOID) — ไม่ผิดกฎ scout (sector ซ้ำได้) แต่ไม่ใช่ top pick ของ round นี้ — ตั้งเป็น secondary candidate. Entry Zone (prelim) $38-45 (mid $41.5) · Stop (prelim) $33 (-20%) | **NEW — Round 27 — รอ Analysis เฉพาะถ้า CIO สนใจ deep-value managed-care bet (CAUTION: ROIC ต่ำ + guidance-withdrawal history, ต้อง Bear-heavy) — Target date 2026-09-10** |
...
| **CNC** | Managed Care | $43.68 | $66.03 | **-33.8%** | EV/FCF 3.13x (~32% eq. yield, ผิดปกติ) / P/B 1.20x | 4.32% | ✅ PASS | ✅ PASS (ต้อง normalize) | ⚠️ WEAK (ROIC<WACC, ใช้ P/B lens แทนตาม OSCR precedent) | **CONDITIONAL PASS — secondary** |
