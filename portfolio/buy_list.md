...
|--------|--------|---------------|---------------------------|------------------|------|--------------|--------------|--------------------------|----------|
| **LVS** | Value | 2026-09-20 | $47.58 / n/a (ESG haircut อยู่ใน scenario ไม่แยกคำนวณ) | **$38–41** ($39.50) | $31.60 | $39.98 (Sept 18 close) | **≈0% (ราคาอยู่ในโซนแล้ว!)** — ติดขัดที่ Conviction gate (6.33<7.0) + Regime TC block ไม่ใช่ราคา | Multiple re-rate ≥9.0x verified / Q3 earnings VIP hold normalize + Macau share ≥26% / China clarification ไม่กระทบ gaming travel spend / MBS reaffirm 2031 no revision | Q3 2026 earnings (21 ต.ค. 2026) |
...
| **DECK** | Value | 2026-09-25 (v2 full 5-subagent pipeline, Morgan QA Round 3 PASS) | $105.45 / n/a (ESG neutral — 12.24 Low Risk, no FV adj) | **$75–85** ($80) — **ราคาปัจจุบันอยู่ในโซนแล้ว** ($78.59–80.15) | $65 | $78.59–80.15 (Sept 25 close) | **≈0% (ในโซน)** — ติดขัดที่ Conviction gate (6.67<7.0) ไม่ใช่ราคา, MOS ผ่านสบาย +34.2% | HOKA growth re-accelerate ≥mid-teens 2Q ติดพร้อม margin expansion (ไม่ใช่ buyback-driven EPS) / FY27 op margin guide กลับ ≥22% บน genuine operating leverage / Tariff resolution ปลด Vietnam-sourcing headwind | Q2 FY2027 earnings (22 ต.ค. 2026) |

> **หมายเหตุ (2026-09-20 — เพิ่ม LVS):** LVS เข้า buy_list วันนี้เป็นกรณีพิเศษ — **ราคาปัจจุบันอยู่ในโซน entry อยู่แล้ว** ($39.98 vs zone $38-41) ต่างจาก 9 ตัวที่เหลือซึ่งทุกตัวยังห่าง entry zone −11% ถึง −50% สาเหตุที่ยังไม่ deploy LVS ไม่ใช่ราคาแพงเกินไป แต่เป็นเพราะ (1) Conviction เฉลี่ย 6.33/10 ยังไม่ถึงเกณฑ์ Value bucket gate ≥7.0 — MOS 16.0% headline พึ่งพา multiple re-rating assumption เดียวที่ยังไม่มี catalyst ยืนยัน (no-rerating scenario MOS เหลือแค่ ~1.75%) (2) Regime TRANSITIONAL-CAUTIOUS ห้าม open new positions ทั้งหมดตาม IPS อยู่แล้ว — **Trigger สำคัญสุดคือ Q3 earnings 21 ต.ค. 2026** ถ้า VIP hold normalize + Macau share ≥26% ยืนยัน + multiple เริ่ม re-rate → conviction น่าจะขยับผ่าน 7.0 ได้เร็วกว่าตัวอื่นในลิสต์มาก เพราะไม่ต้องรอราคาย่อเพิ่ม
>
> **หมายเหตุเดิม (2026-09-13, Portfolio Review Mode 3):** ตลาด rally แรงวันที่ 11 ก.ย. (ADI +4.85%, QCOM +2.88%, ACN +3.37%) ทำให้ gap ของ ADI/QCOM/ACN **ห่างขึ้น** ขณะที่ FSLR/ZETA/MELI/CHYM **แคบลงเล็กน้อย** และ GCT ยังคงห่างขึ้นต่อเนื่อง (−7%→−11%) เพราะราคาวิ่งขึ้นไม่หยุด GCT ยังเป็นตัวใกล้ entry ที่สุดในกลุ่มเดิม (−11%) ก่อน LVS เข้ามาเป็นตัวใกล้ entry ที่สุดจริงในลิสต์ทั้งหมดตอนนี้ (≈0%). ถ้า GCT ย่อถึง ~$46 + Q3 2026 (~ต้น พ.ย.) ยืนยัน OCF-NI conversion ฟื้น + conviction lift ≥6.5 → deploy candidate. **8/9 ตัวเดิม (ADI/FSLR/ZETA/MELI/CHYM/OSCR/QCOM/ACN) ต้องย่อ −38% ถึง −50%** — realistic เฉพาะใน correction −15–20% (มา ~1 ครั้ง/4–6 ปี) หรือ thesis-specific catalyst (ACN Q4 FY26 earnings). **Regime TRANSITIONAL-CAUTIOUS → Force Deploy SUSPENDED** (ยืนยันซ้ำโดย Atlas re-call 2026-09-20 หลัง FOMC — override maintained ไม่ flip กลับ RISK-ON) — แม้ราคาแตะ entry zone ก็ยัง**ห้าม deploy ใหม่**จนกว่า Atlas re-call RISK-ON. **FV ages:** LVS = 0 วัน (fresh) · ADI/MELI/ZETA/CHYM/OSCR/GCT = 21–22 วัน (asof 2026-09-20) · FSLR = 17 วัน · QCOM = 15 วัน · ACN = 9 วัน — ยังไม่มีตัวไหนเกิน 90 วัน (ไม่ต้อง forced re-verify) แต่ GCT ใกล้ entry → Emma lightweight FV re-verify จะ trigger ก่อน deploy ถ้าราคาแตะ $46.
