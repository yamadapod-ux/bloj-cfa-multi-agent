...
- **Sensitivity Matrix** *(House Rule — 2026-04-24)*: Quinn ต้องสร้าง 5×5 matrix เลือก 2 ตัวแปรสำคัญที่สุดของหุ้นนั้น แสดง fair value ทุก scenario | ตัวอย่าง: NVDA = Revenue Growth × WACC | CCJ = Uranium Price × WACC | AVGO = Revenue Growth × Operating Margin | RKLB = Neutron Cadence × Space Systems GM | **AMD = Revenue CAGR × WACC** | **MRVL = Revenue CAGR × Terminal EBIT Margin** | **VEEV = Revenue CAGR × WACC** | **LDOS = Revenue CAGR × EBITDA Margin** | **FICO = Revenue CAGR × DOJ Penalty Probability** | **QCOM = Revenue CAGR × China Revenue % Lost**
...
- **Bear 25% (ไม่ใช่ 40%) Confirmed for AVGO** — AVGO = quality name with binary overlay (VMware churn, Apple, Hock Tan) แต่ไม่ใช่ existential binary (RKLB/LDOS/FICO = Bear 40%); IPS 2026-05-15 rule applied correctly
...
- Cross-sectional Relative Valuation / Peer Comp [L2]: เทียบ multiple ของหุ้นกับ peer set ในมิติเดียวกัน — *(รอ CIO confirm — used 10 times)*
- Investment Thesis Construction with Bull/Bear Flip Triggers [L3]: framework กำหนดเงื่อนไขวัดได้ที่จะเปลี่ยน rec — *(รอ CIO confirm — used 10 times)*
- ESG Materiality Analysis [L2-3]: ระบุ ESG factor ที่กระทบ valuation จริง พร้อม % impact — *(รอ CIO confirm — used 12 times)*
...
- **Forward Sharpe as Primary "No Deploy" Signal [L1/L3]**: Forward Sharpe = expected excess return ÷ forward vol; ถ้า < S&P 500 benchmark (~0.55) = ไม่ควร deploy capital ใหม่ (ดีกว่าถือ SPY); AVGO v2 Forward Sharpe 0.12 = primary signal สำหรับ HOLD — *(รอ CIO confirm — used in AVGO v2 + QCOM — 2nd use as explicit signal)*
